...
export default function EditSupplierComponent({ id }: { id: string }) {
  ...
  async function onSubmit(data: z.infer<typeof FormSchema>) {
    try {
      const res = await axiosInstance.patch(`${ENV.API_URL}/suppliers/${id}`, {
        ...data,
      });
  ...
  const fetchSupplier = useCallback(async () => {
    try {
      const res = await axiosInstance.get(`${ENV.API_URL}/suppliers/${id}`);
      const data: Supplier = res.data.data.supplier;
      form.reset({
        ...data,
      });
    } catch (err) {
